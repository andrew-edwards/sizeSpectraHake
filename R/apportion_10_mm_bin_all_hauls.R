...
id_with_10_mm_but_no_1_mm <- setdiff(id_with_10_mm, id_with_1_mm_and_10_mm)

# Had done this (think before keeping resolution column all the way through).
# If has a _B then by definition is a two-set haul.
# id_set_with_B <- unique(counts_per_bin_per_set_id_one_year$id_set)[grep("_B", unique(counts_per_bin_per_set_id_one_year$id_set))]
# id_set_with_B
# id_with_B <- sub("_B", "", id_set_with_B)
# id_with_B

# Just two-set hauls
# counts_per_bin_one_year_repeated_hauls <- filter(counts_per_bin_per_set_id_one_year,
#                                                 id %in% id_with_B)
# replaces this now I kept id in the tibble
## counts_per_bin_one_year_repeated_hauls_2 <- filter(counts_per_bin_per_set_id_one_year,
##                                              id_set %in% c(as.factor(sub("_B",
##                                                                          "_A",
##                                                                          id_set_with_B)),
##                                                            id_set_with_B))

# counts_per_bin_one_year_repeated_hauls %>% a()
...
for(i in 1:length(id_with_1_mm_and_10_mm)){
  ...
  counts_per_10_mm_bin_all_apportioned <- bind_rows(counts_per_10_mm_bin_all_apportioned,
    counts_per_10_mm_bin_this_haul_apportioned)
}
...
# bin, so just leave the repeated calculation (not too time consuming)
...
for(i in 1:length(id_with_10_mm_but_no_1_mm)){
  ...
  # Get the 10-mm apportioned calculation for this haul
  counts_per_10_mm_bin_this_haul_apportioned <-
    apportion_10_mm_bin(counts_per_bin_1_mm_one_year,
                        counts_per_bin_this_haul %>% filter(resolution == 10),
                        xmin = xmin)
...
counts_per_10_mm_bin_all_apportioned %>% a()
