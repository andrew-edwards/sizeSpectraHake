---
title: "Analyses of hake lengths using size spectra approach -- data wrangling of original and updated Nov 2023 data"
...
This is `hake-lengths-wrangling-2.Rmd` as now need also SURVEY OR CRUISE, which
I hadn't realised for Nov 23 data (original data set did not have a mix of
surveys), and that's a bit of a major change.

Not updating original data set analysis, here just Nov 23.
...
raw_simp_nov23 <- select(raw_nov23,
  ...
  mutate(id_cruise = paste(year,       # Adapting Hayden's id as now need cruise also. To make calcs easier
                    strata,     # than grouping, and to more easily compare any results.
                    cruise,
                    haul_no,
                    sep = "_"),
         id_survey = paste(year,   # try survey
                    strata,
                    survey,
                    haul_no,
                    sep = "_"),
         haul_no = as.factor(haul_no),
         id_cruise = as.factor(id_cruise),
         id_survey = as.factor(id_survey)) %>%
  arrange(year,
          strata,
          haul_no)
...
summary(raw_simp_nov23)

# Check if id_cruise and id_survey are essentially the same. Kind of looks like
# it might be from summary. id_survey is easier to read.

# Do both ways and check they're the same
confirm_c <- summarise(group_by(raw_simp_nov23,
                                id_cruise),
                       id_survey = unique(id_survey))
# no warning suggests good

confirm_s <- summarise(group_by(raw_simp_nov23,
                                id_survey),
                       id_cruise = unique(id_cruise))
# gives a warning, so try reframe

confirm_s_reframe <- reframe(group_by(raw_simp_nov23,
                                      id_survey),
                             id_cruise = unique(id_cruise))
confirm_c
confirm_s
...
# Don't understand how all the same size, if confirm_s gave a warning. Aha:

summary(confirm_c)
summary(confirm_s)
summary(confirm_s_reframe)

# So 6 repeated id_surveys I think, whereas HERE HERE

HERE HERE - check
id's are now good, and check if can just do by survey instead, which will be simpler
