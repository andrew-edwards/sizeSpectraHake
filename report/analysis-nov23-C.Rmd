...
Confused as to what data are -- need to go back to data, see this:
...
### Check same idea with 2016 data that look to have no 10-mm issue

Copy above code for 2004 and replace with 2016. Obviously functionalise if worth
doing at some point.

```{r raw2016}
raw_2016 <- filter(raw_simp_prop,
                   year == 2016,
                   strata %in% strata_to_analyse)
raw_2016
summary(raw_2016)
# filter(raw_2016, prop_measured < 0.01) %>% as.data.frame()

filter(raw_2016, x == 30) %>% select(-"strata")

# Vector of measured lengths that aren't multiples of 10 mm
non_10_mm_measurements <- unique(raw_2016$x)[unique(raw_2016$x) %% 10 > 0] %>% sort()

non_10_mm_measurements

# ids which do not have non-10-mm measurements; i.e. only have 10 mm ones (the
# 1-mm ids can still have some 10-mm measurements)
ids_with_mm_accuracy <- filter(raw_2016,
                               x %in% non_10_mm_measurements)$id %>%
                                                            unique()
raw_with_10_mm_only <- filter(raw_2016,
                              !(id %in% ids_with_mm_accuracy))
raw_with_10_mm_only %>% a()
```

So there aren't any 10-mm only in 2016, as expected from visually seeing the
histogram. So code works for empty - HERE so functionalise this check so can do
for each year.

END of 2016 based on 2004 analysis
...
# Vector of measured lengths that aren't multiples of 10 mm
non_10_mm_measurements <- unique(raw_simp_prop_strata$x)[unique(raw_simp_prop_strata$x) %% 10 > 0] %>% sort()
...
raw_with_10_mm_only %>% group_by(id) %>% summarise(year = unique(year), n()) %>% a()
```
