...
Now do the data preparation first, maybe no need to functionalise.
```{r dataprep}
...
min_wmin_to_consider_1_mm <- 27.5  # 27.5 is what Iain had said; trying 25 for
                                 # simplicity now
...
# from wrangling .Rmd; if has a _B then by definition is a two-set haul
...
copied list of steps to new function

```{r xmin10mm}
# Think these were what I based the above code on, not sure needed. Still useful
# to do though.
## id_table_repeated_index <- which(table(counts_per_bin_per_set_id_one_year$id) > 1)
...
# This works, then do the list of steps to work out what to do with first 10-mm bin.
# This sums the counts in each bin.
counts_per_bin_pre_apportion <- summarise(group_by(counts_per_bin_per_set_id_one_year,
                                                   wmin,
                                                   resolution),
                                          number = sum(number),
                                          wmax = unique(wmax)) %>%
  ...
  relocate(wmin,
           ...
           resolution) %>%
  ungroup()
counts_per_bin_pre_apportion %>% a()

# counts_per_bin_old_pre_apportion <- counts_per_bin   # TODO temp while doing
# function
...
counts_per_bin_1_mm <- filter(counts_per_bin_pre_apportion,
                              resolution == 1,
                              wmin >= min_wmin_to_consider_1_mm - 0.001,
                              wmax <= max_wmax_to_consider_1_mm + 0.001)
counts_per_bin_1_mm %>% a()

xmin <- determine_xmin(counts_per_bin_1_mm)
```
...
```{r function10mmbin}
# input will be:
# counts_per_bin_1_mm, either from same haul if it exists, or from
#   full year combined
# counts_per_bin_10_mm that's to have bin containing xmin appropriately
# apportioned; keep all the others (including lowers ones) the same
# xmin, as calculated above (write a function for that)
...
### Step 2
```{r step2}
# Do for each haul with two sets
counts_per_bin_by_haul_to_replace <- tibble()    # Going to be the values we
                                        # need to sum and put
                                        # into counts_per_bin, replacing each
                                        # existing wmin* to wmax* one.
# LOOP for id_with_B, then a loop for 10-mm sets without corresponding 1-mm bin,
# just have to give different input into apportion_...(). Then functionalise
# this also.

for(i in 1:length(id_with_B)){
  # Should make this into a function TODO
  counts_per_bin_this_haul <- filter(counts_per_bin_one_year_repeated_hauls,
                                     id == id_with_B[i])
...
apportion_10_mm_bin
  ...
  bin_10_mm_contains_xmin <- filter(counts_per_bin_this_haul,
                                    resolution == 10,
                                    wmin < xmin,
                                    wmax > xmin)
  ...
  # Step 3, get the 1-mm bins fully contained in that 10-mm bin
  if(nrow(bin_10_mm_contains_xmin) == 1){
    bin_1_mm_in_bin_10_mm_contains_xmin <- filter(counts_per_bin_this_haul,
                                                  resolution == 1,
                                                  wmin > bin_10_mm_contains_xmin$wmin,
                                                  wmax < bin_10_mm_contains_xmin$wmax)
    bin_1_mm_in_bin_10_mm_contains_xmin     # Has to be at least one because
                                            # xmin is a 1-mm wmin by defintion
    ...
    # Step 5
    # wmin* to xmin bin (won't be needed but used for double checking):
    ...
    # Append new values to those for previous hauls (done in earlier iterations
    # of this loop).
    counts_per_bin_by_haul_to_replace <- bind_rows(counts_per_bin_by_haul_to_replace,
                                                   new_bins_for_this_haul_1,
                                                   new_bins_for_this_haul_2)
  }  # Ends if(nrow(bin_10_mm_contains_xmin) == 1)

  # Else we have nrow(bin_10_mm_contains_xmin) == 0 and so don't have to deal
  # with the 10-mm bin (all the 10-mm bins might be higher, for example)
}

counts_per_bin_by_haul_to_replace

# Check no negatives
expect_equal(filter(counts_per_bin_by_haul_to_replace, number <0) %>% nrow(), 0)

# Now sum to give just two new bins (as done above for counts_per_bin_pre_apportion), that will replace the existing one in counts_per_bin_pre_apportion
counts_per_bin_to_replace <- summarise(group_by(counts_per_bin_by_haul_to_replace,
                                                wmin,
                                                resolution),  # should only be 10 anyway
                                       number = sum(number),
                                       wmax = unique(wmax)) %>%
  arrange(resolution,
          wmin) %>%
...
expect_equal(sum(orig$number),
             sum(counts_per_bin_to_replace$number))

# Have to replace the counts for those hauls only.
```

BUGGER, there must be 10-mm hauls for those bins also, but with no corresponding
1-mm measurements we can use the above trick for. Do we just use the same
proportions? May have to. Or do the same trick based on proportions based on all
the data (excluding the repeated ones). Maybe see how the proportions differ,
and may have to do a sensitivity to both. TODO next functionalise the proportion
stuff, to make it more general. Getting confusing here with so many tibbles.

Check:
```{r checksomething}

filter(counts_per_bin_per_set_id_one_year,
       !(id %in% id_with_B),
       resolution == 10,
       wmin == 25)
...
# need this at some point I think. haven't edited after doing new function

counts_per_bin_one_year_repeated_hauls

# not needed yet:
counts_per_bin_10_mm <- filter(counts_per_bin_pre_apportion,
                               resolution == 10,
                               wmin >= min_wmin_to_consider_10_mm - 0.001,
                               wmax <= max_wmax_to_consider_10_mm + 0.001)

counts_per_bin_10_mm %>% a()




# Then can plot the histograms:
make_hist_1_mm <- make_hist(counts_per_bin_1_mm)  # adapted function to be able
                                        # to use  wmin, wmax etc (as for MLEbins).
make_hist_10_mm <- make_hist(counts_per_bin_10_mm,
                             bin_width = 10)
...
## HERE HERE HERE - have to deal with the 1mm and 10mm bins. Aha - need to use
...
n = sum(counts_per_bin_desc$number)
# xmin = min(counts_per_bin_desc$wmin) # done earlier
xmax = max(counts_per_bin_desc$wmax)
...
MLEbins_one_year <- calcLike(negLL.fn = negLL.PLB.bins.species,
                             ...
                             dataBinForLike = counts_per_bin_desc,
                             n = n,
                             xmin = xmin,
                             xmax = xmax,
...
MLEbins_one_year
# woo-hoo, matches analysis-nov23-C-before-resolving-10mm.pdf for 1994
```
