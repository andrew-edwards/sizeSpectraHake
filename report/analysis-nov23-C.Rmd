...
TODO - do one year first, then change `fit_all_years` to loop over all years.
...
```{r oneyear}
one_year = 2001
counts_per_bin_one_year <- filter(counts_per_bin_per_set_id,
                              year == one_year)

# from wrangling .Rmd; if has a _B then by definition is a two-set haul
id_set_with_B <- unique(counts_per_bin_one_year$id_set)[grep("_B", unique(counts_per_bin_one_year$id_set))]
...
# Just two-set hauls
counts_per_bin_one_year_repeated_hauls <- filter(counts_per_bin_one_year,
                                                 id %in% id_with_B)
# replaces this now I kept id in the tibble
## counts_per_bin_one_year_repeated_hauls_2 <- filter(counts_per_bin_one_year,
...
2. determine which 10-mm bin contains that xmin, wmin^* to wmax^* i.e. 25-35
...
# Think these were what I based the above code on, not sure needed.
## id_table_repeated_index <- which(table(counts_per_bin_one_year$id) > 1)
...
counts_per_bin <- summarise(group_by(counts_per_bin_one_year,
                                     wmin,
                                     spec_code),
                            number = sum(number),
                            wmax = max(wmax)) %>%
...
for(i in 1:length(id_with_B)){
  ...
  if(nrow(bin_10_mm_contains_xmin) == 1){
    ...
    bin_1_mm_in_bin_10_mm_contains_xmin
    ...
    ratio_gt_xmin_versus_lt_xmin <- number_gt_xmin / number_lt_xmin
    # Step 5
HERE - need to work out what to replace in the counts
...
## MLEbins and see vignette. But 1994 has none of those, summary(counts_per_bin_one_year)
...
```{r fitallyearsfunction, cache = FALSE}
