...
counts_per_bin_per_set_id <- summarise(group_by(counts_full,
                                       ...
                                       spec_code = unique(resolution),
                                       number = sum(binCount),
                                       wmin = unique(wmin),
                                       wmax = unique(wmax),
                                       id = unique(id),   # makes the 2-set
                                         # hauls easier
...
# This works, then do above steps to work out what to do with first 10-mm bin.
# This sums the counts in each bin.
counts_per_bin <- summarise(group_by(counts_per_bin_per_set_id_one_year,
                                     wmin,
                                     spec_code),
                            number = sum(number),
                            wmax = unique(wmax)) %>%
...
# Need to separate 1mm and 10mm:
counts_per_bin_1_mm <- filter(counts_per_bin,
                              specCode == 1,
...
# Now sum to give just two new bins (as done above for counts_per_bin), that will replace the existing one in counts_per_bin
counts_per_bin_to_replace <- summarise(group_by(counts_per_bin_by_haul_to_replace,
                                                wmin,
                                                spec_code),  # should only be 10 anyway
                                       number = sum(number),
                                       wmax = unique(wmax)) %>%
  arrange(spec_code,
          wmin) %>%
...
# need this at some point I think

counts_per_bin_one_year_repeated_hauls

# not needed yet:
counts_per_bin_10_mm <- filter(counts_per_bin,
                               specCode == 10,
