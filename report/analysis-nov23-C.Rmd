...
Now fit one year, say 1994:
```{r oneyearfit}
counts_per_bin_year <- filter(counts_per_bin_per_set_id,
                              year == 1994)
counts_per_bin <- summarise(group_by(counts_per_bin_year,
                                     wmin,
                                     spec_code),
                            number = sum(number),
                            wmax = max(wmax)) %>%
...
max_ind <- which.max(counts_per_bin$Number)
max_ind

if(max_ind == 1){
  counts_per_bin_desc <- counts_per_bin} else {
...
## MLEbins and see vignette. But 1994 has none of those, summary(counts_per_bin_year)
## But gives the right answer. So then make loop to go over years here, then
...
## don't make much difference to picking xmin.
