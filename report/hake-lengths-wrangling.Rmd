...
# TODO check prop_measured is the correct thing - thinking it might not be now
# as done earlier. Aha - may need to be done in ids_sets. Check earlier - think
# it is correct for each set, and that's what we need.
```
...
of earlyer. Actually, think still need to work out the 10-mm issue, but doing it
...
Just looking at previously saved version that goes into analysis, to check
formatting, though should be at the end of this current file anyway
```{r previouslysaved}
# raw_simp_prop_nov_23  - this was used. But going to rewrite `fit_all_years`
# anyway, add the bin widths here
# raw_simp_prop <- readRDS("raw_simp_prop_nov23.rds")

# Think save raw_simp_prop_nov23_sets_resolution  so have full data set in
