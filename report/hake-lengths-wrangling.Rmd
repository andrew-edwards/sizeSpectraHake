...
NOT SURE NEED THIS ANY MORE TODO, ignoring for now
```{r checknumber5}
...
# TODO need bin widths from earlier, or later
...
HERE - need to get sets appended to `raw_simp_prop_nov23` so can do the bin
widths. Then mutate to give wmin and wmax for each row.

HERE TUES NIGHT `summarise` should work if group by set also, but need to




This was just for checking the repeated id's. Decide if still needed; think may
have been to help figure out what I've now done above.
```{r checknumber6, eval=FALSE}
id_repeated_full <- filter(raw_simp_prop_nov23,
                           id %in% id_repeated$id)
id_repeated_full_unique <- reframe(group_by(id_repeated_full,
                                            id,
