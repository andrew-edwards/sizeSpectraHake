...
**Saving this as `hake-length-wrangling-3b1ee22.pdf` with the figuring out of two sets in a haul, then simplifying down
again going forward**. Note that `id_repeated_2` here will be the `id_repeated`
in future .pdfs.
...
Work out where to do the fix to deal with `2022_C_5`, repeating
the above but for `total_no` also, should have any that have different
`total_no` but the same `number_measured` (`2022_C_5` is the opposite, so
doesn't change):
```{r checknumber99}
check_number_measured_2 <- reframe(group_by(raw_simp_prop_nov23,
                                            id),
                                   number_measured = unique(number_measured),
                                   total_no = unique(total_no),
                                   number_rows = n())
check_number_measured_2
```
This has four more rows than `check_number_measured`. The ids are
```{r idstworows2}
id_repeated_ind_2 <-
  table(check_number_measured_2$id)[which(table(check_number_measured_2$id) > 1)]
id_repeated_2 <- filter(check_number_measured_2,
                      id %in% names(id_repeated_ind_2))
id_repeated_2
droplevels(id_repeated_2$id) %>%
  table() %>%
  range()     # shows only 2's, no 3's.
```
The new ids that we didn't have in `id_repeated_ind` are:
```{r idstworows3}
id_repeated_2[which(!(id_repeated_2$id %in% id_repeated$id)), ]

# And the other way around, has none:
id_repeated[which(!(id_repeated$id %in% id_repeated_2$id)), ]
```

So `2022_C_5` is in both, because `total_no` is the same for each sample.

We now try the third option which is grouping only by `total_no`:
```{r checknumber98}
check_number_measured_3 <- reframe(group_by(raw_simp_prop_nov23,
                                            id),
                                   total_no = unique(total_no),
                                   number_rows = n())
check_number_measured_3
```
This has three more rows than `check_number_measured` and one less than
`check_number_measured_2`, which makes sense (`check_number_measured_2` has
`check_number_measured` plus any new ones here, which should drop out as
`2022_C_5`:
```{r idstworows3a}
id_repeated_ind_3 <-
  table(check_number_measured_3$id)[which(table(check_number_measured_3$id) > 1)]
id_repeated_3 <- filter(check_number_measured_3,
                        id %in% names(id_repeated_ind_3))
id_repeated_3
droplevels(id_repeated_3$id) %>%
  table() %>%
  range()     # shows only 2's, no 3's.
```
The new ids that we didn't have in `id_repeated_ind` are:
```{r idstworows4}
id_repeated_3[which(!(id_repeated_3$id %in% id_repeated$id)), ]

# And the other way around, has just 2022_C_5
id_repeated[which(!(id_repeated$id %in% id_repeated_3$id)), ]
```
Last is correctly the one we had not originally caught.

So `id_repeated_2` should have everything, so these should both be empty:
```{r idcheck}
id_repeated_2[which(!(id_repeated$id %in% id_repeated_2$id)), ]
id_repeated_2[which(!(id_repeated_3$id %in% id_repeated_2$id)), ]
```

Great. `id_repeated_2` is what we want to use going forward.

Going to save this pdf as commit after `3b1ee22` (commit after that fills in the
commit has here), and call it
`hake-lengths-wrangling-3b1ee22.pdf`. Then going to change
`id_repeated` to what we want it to be (i.e. what `id_repeated_2` is
here). So will have the .pdf for the explanations, but now we've figured it out
don't need all the details cluttering up the wrangling.
HERE HERE HERE
