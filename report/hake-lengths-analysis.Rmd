...
extract that now and check agrees with the saved version.
```{r newdata}
# Select one of these   TODO should really do this later, do as an option in
# fitting function.

# strata_to_use <- c("C", "NC", "S", "SC", "N")
strata_to_use <- "C"
...
raw_simp_new <- select(raw_new,
  ...
  filter(x >= 28,
         x < 90,
         strata %in% strata_to_use) %>%   # Should really do this later
...
```
HERE HERE HERE - last thing gives no error (just strata C right now). Excellent,
is what I thought.

Should be good to move onto analysis??? Check have all the columns and remove
un-needed ones. HERE HERE HERE.
...
## Updated data set from John Field, December 2023

Copying loading in from above - want to be able to c
