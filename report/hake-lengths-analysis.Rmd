...
fit_all_years <- function(raw_simp_prop,
                          strata = c("C", "NC", "S", "SC", "N"), # already done
                          # above but leave as default
                          bin_width_each_year    # tibble of bin widths in each
                                        # year, calculated earlier
                        ){
  ...
  for(i in 1:length(full_years)){
    ...
    if(!expect_equal(min(diff(counts_per_bin$binMid)), bin_width)){
      ...
      # Having no adjancet bins with values will cause this to fail, which seems
...
# For testing:
res_just_2021 <-  fit_all_years(filter(raw_simp_prop, year == 2021),
                                bin_width_each_year = bin_width_each_year)
```
...
plot.hake_spectra_results <- function(res,
                                      years = NULL,  # years to be plotted
                                      ...){
...
Just 2021 for debugging histogram (running in R console)
```{r just2021}
